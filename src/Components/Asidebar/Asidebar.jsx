...
const Asidebar = ({ isOpen, handleOpenSideBar }) => {
  ...
  return (
    <>
      <aside
        className={` hidden md:flex flex-col  transition-all duration-300 ease-linear gap-3 ${
      ...
      >
        ...
        <Link
          href={`/dashboard/${asideData[0].dashboard.pathname}`}
          className="dashboard h-[10vh] flex px-4 py-3 text-white gap-3 bg-[#5852FE]  items-center"
        >
          <span className={`text-[22px] ${isOpen && "mx-auto"} `}>
            {asideData[0].dashboard.icon}
          </span>
          {!isOpen && (
            <span className="capitalize">{asideData[0].dashboard.link}</span>
          )}
        </Link>
        <div className="menu h-[45vh] flex flex-col gap-4 px-4">
          {asideData[0].menu.map((asd, i) => (
            <Link
              href={`/dashboard/${asd.pathname}`}
              key={i}
              className={`menu  w-full flex-row flex gap-3 items-center`}
            >
              <span
                className={`text-[22px] ${isOpen && "mx-auto"} text-[#5852FE] `}
              >
                {asd.icon}
              </span>
      ...
      <button
        ...
        className={`lg:hidden mt-2 absolute border rounded-full z-[500] ${
          openMobileMenu ? "toggle open  left-[210px]" : "toggle left-[14px]"
        }`}
      >
      ...
      <aside
        className={`flex flex-col  transition-all duration-300 ease-linear gap-3 ${
          !openMobileMenu ? "w-0 overflow-hidden" : "w-[250px]"
        } border-r min-h-screen fixed top-0 left-0 bottom-0 bg-white`}
      >
        <div className="logo flex h-[10vh] w-full items-center px-3 relative justify-between">
          <Image
