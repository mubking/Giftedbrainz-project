...
function Competition() {
  ...
  const handleAnswerSubmit = () => {
    ...
    if (nextQuestion < quizData.length) {
      ...
      toast.success(
        "Quiz completed! Your total score: " +
          score +
          "/" +
          quizData.length * 10
      );
  ...
  return (
    <div className=" flex flex-col h-screen w-full  justify-center items-center  ">
      <div className="text-center text-black text-[25px] font-semibold font-['Poppins'] capitalize">
        question 1 - 15
      </div>
      <div className="text-center text-black  text-opacity-60 text-2xl font-semibold font-['Poppins'] capitalize">
        note: choose your answer below and <br /> press continue once done.
      </div>
      <div className="w-[70%] h-[40vh] bg-zinc-300 rounded-[20px] text-center flex flex-col items-center justify-center">
        <div className="text-2xl">
          Question {currentQuestion + 1} of {quizData.length}
        </div>
        <div className="text-3xl">{quizData[currentQuestion].question}</div>
      </div>
